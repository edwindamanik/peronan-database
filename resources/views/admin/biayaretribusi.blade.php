...
<main>
    <div class="container-fluid">
        <h2 class="mt-4">Daftar Biaya Retribusi</h2>
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb">
                <li class="breadcrumb-item active" aria-current="page">Daftar Biaya Retribusi</li>
            </ol>
        </nav>
        <button type="button" class="btn btn-primary" data-toggle="modal" data-target="#myModalTambah">
            Tambah Biaya Retribusi
        </button>
        <div class="card mb-4">
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-header">
                    <h5 class="modal-title">Tambah Biaya Retribusi</h5>
                    <button type="button" class="close" data-dismiss="modal">&times;</button>
                </div>

                <!-- Modal body -->
                <div class="modal-body">
                    <form action="{{ URL('/biaya-retribusi/store') }}" method="post">
                        @csrf
                        <div class="form-group">
                            <label for="inputText">Kelompok Pasar</label>
                            <select class="form-control" id="selectOption" name="kelompokPasar" required>
                                <option value="">Pilih Kelompok Pasar</option>
                                @foreach ($kelompok_pasar as $itemKelompokPasar)
                                    <option value="{{ $itemKelompokPasar->id }}">{{ $itemKelompokPasar->kelompok_pasar }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="form-group">
                        ...
                        <div class="form-group">
                            <label for="inputText">Pilih Satuan</label>
                            <select class="form-control" id="selectOption" name="satuan" required>
                        ...
                        <div class="form-group">
                            <label for="inputText">Harga Satuan</label>
                            <input type="number" class="form-control" name="hargaSatuan" placeholder="Harga Satuan" required>
                        </div>
                        <div class="form-group">
                            <label for="inputText">Harga Total</label>
                            <input type="number" class="form-control" name="harga" placeholder="Harga Total" required>
                        </div>
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form action="" method="POST" id="editForm">
                        ...
                        <div class="form-group">
                            <label for="inputText">Kelompok Pasar</label>
                            <select class="form-control" id="kelompokPasar" name="kelompokPasar" required>
                                <option value="">Pilih Kelompok Pasar</option>
                                @foreach ($kelompok_pasar as $itemKelompokPasar)
                                    <option value="{{ $itemKelompokPasar->id }}">{{ $itemKelompokPasar->kelompok_pasar }}</option>
                                @endforeach
                        ...
                        <div class="form-group">
                            <label for="inputText">Harga Total</label>
                            <input type="number" class="form-control" id="harga" name="harga" placeholder="Harga Total" required>
                        </div>
                        <button type="submit" class="btn btn-primary">Submit</button>
                    </form>
                </div>
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <div class="modal-footer d-flex justify-content-center">
                    ...
                    <a href="/biaya-retribusi/delete/{{ $item->id }}" class="btn btn-danger delete-button">Delete</a>
                </div>
            </div>
        </div>
    </div>
...
<script>
    ...
    $(document).ready(function () {
        $('.edit-button').click(function () {
            ...
            $('#hargaSatuan').val(jsonData.harga_satuan);
            $('#harga').val(jsonData.harga);

            // Set selected option in the #pasar select dropdown
            $('#kelompokPasar option').each(function () {
                if ($(this).text() === jsonData.kelompok_pasar.toString()) {
            ...
            var actionUrl = '/biaya-retribusi/update/' + jsonData.id;
    ...
    $(document).ready(function () {
        $('#myModalDelete').on('show.bs.modal', function (event) {
            ...
            deleteButton.attr('href', '/biaya-retribusi/delete/' + pasarId);
        });
    });
